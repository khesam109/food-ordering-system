...
DROP TYPE IF EXISTS "food_order".saga_status;
CREATE TYPE "food_order".saga_status AS ENUM ('STARTED', 'FAILED', 'SUCCEEDED', 'PROCESSING', 'COMPENSATING', 'COMPENSATED');

DROP TYPE IF EXISTS "food_order".outbox_status;
CREATE TYPE "food_order".outbox_status AS ENUM ('STARTED', 'COMPLETED', 'FAILED');

DROP TABLE IF EXISTS "food_order".payment_outbox CASCADE;
CREATE TABLE "food_order".payment_outbox
(
    id uuid NOT NULL,
    ...
    outbox_status "food_order".outbox_status NOT NULL,
    saga_status "food_order".saga_status NOT NULL,
    order_status "food_order".order_status NOT NULL,
...
DROP TABLE IF EXISTS "food_order".restaurant_approval_outbox CASCADE;
CREATE TABLE "food_order".restaurant_approval_outbox
(
    id uuid NOT NULL,
    saga_id uuid NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE NOT NULL,
...
CREATE INDEX "restaurant_approval_outbox_saga_status"
    ON "food_order".restaurant_approval_outbox
        (type, outbox_status, saga_status);
